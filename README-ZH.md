...
<p align="center">
  ...
  <a href="#命令列-cliidm"><img src="https://img.shields.io/badge/命令列-190%20channels-238636?style=flat-square" alt="命令列"></a>
...
<p align="center">
  <img src="./src/assets/screen/7.png" alt="時間軸流程——角色聖經、場景與道具同一張圖" width="100%">
</p>
<p align="center"><em>時間軸——角色聖經、場景、道具同一張流程圖；下面仍是片段軌道。</em></p>

<table>
  <tr>
    <td width="50%" valign="top">
      <img src="./src/assets/screen/10.png" alt="漫畫成圖頁：完成頁與兩套出片方案" width="100%">
      <p align="center"><em>漫畫——鎖定印刷頁成圖，再選本頁動畫或短劇分鏡。</em></p>
    </td>
    <td width="50%" valign="top">
      <img src="./src/assets/screen/12.png" alt="劇照成圖：四種出圖方式與宣傳靜圖" width="100%">
      <p align="center"><em>劇照——封面、劇照、宣傳、大頭。四種出圖方式，然後是成圖。</em></p>
    </td>
  </tr>
</table>
...
| 桌面 | 遠控 | 命令列 |
|---|---|---|
| Linux · Windows · macOS（Electron） | 瀏覽器操作同一專案 | `instant-drama`——**190** 個 channel，與應用相同 |
...
截圖來自實際應用（`src/assets/screen/`，v1.11.3）。上方主圖是 **時間軸**、**漫畫** 與 **劇照** 三張工作檯。

### 1. 故事管理

封面、草稿狀態、角色／場景／道具／片段數量、搜尋、**匯出備份**／**匯入故事備份**。

![故事管理](./src/assets/screen/1.png)

### 2. 故事編輯 · 基本

封面、AI 填寫風格備註、標題、藝術風格，以及 **生成規則**（必須／禁止）。

![故事編輯基本](./src/assets/screen/2.png)

### 3. 故事章節

先寫正文。可用 AI 生成或潤飾章節；之後填庫時以章節為劇情來源。

![故事章節](./src/assets/screen/13.png)

### 4. 劇情段落

把章節拆成可拍短片。每段綁定角色、場景、道具與動作。

![劇情段落](./src/assets/screen/14.png)

### 5. 選角／場／道具

瀏覽全域庫，只把本故事用到的資產加入。

![選角場道具](./src/assets/screen/15.png)

### 6. 角色庫

全域角色：多圖卡片，按性別／風格／靈魂／語言篩選。

![角色庫](./src/assets/screen/3.png)

### 7. 角色聖經

身份、體型、戲服、細節靜圖，加上書面檔案與生成規則。

![角色聖經](./src/assets/screen/4.png)

### 8. 角色參考圖方案

三格聖經、面容鎖定、戲服板——先選方案再出圖。

![角色參考圖方案](./src/assets/screen/19.png)

### 9. 戲服試穿

選穿着角色、體型底圖與姿勢。成圖會寫入戲服庫與角色庫。

![戲服試穿](./src/assets/screen/16.png)

### 10. 場景板圖

建立鏡頭、主視覺、室內、天氣——同一場景可換氛圍。

![場景板圖](./src/assets/screen/17.png)

### 11. 道具靜圖

主視覺、四分之三、細節、材質、比例。

![道具靜圖](./src/assets/screen/18.png)

### 12. 漫畫 · 排板

規則格與不規則漫畫格。開本（9:16／1:1／16:9）跟模板走。

![漫畫排板模板](./src/assets/screen/9.png)

### 13. 漫畫 · 成圖

兩套出片方案：按印刷頁做動畫，或當短劇分鏡拍。

![漫畫成圖與出片方案](./src/assets/screen/10.png)

### 14. 劇照 · 選題材

封面、海報、劇照、宣傳、社交、大頭、半身、群像。

![劇照題材與開本](./src/assets/screen/11.png)

### 15. 劇照 · 出圖方式

全新生成、用此圖改、鎖定面貌重畫、延續上一張。一鍵可設為故事封面。

![劇照出圖方式](./src/assets/screen/12.png)

### 16. MediaGen · 準備出片

上一段靜圖、本段靜圖、角色、場景，再以文字鎖定連續。

![MediaGen 出片準備](./src/assets/screen/8.png)

### 17. 時間軸軌道

吸附、6／10 秒片段、綁定資產、段落劇本、單段生成。

![時間軸軌道](./src/assets/screen/5.png)

### 18. 時間軸流程

同一套段落畫成圖：角色 → 場景 → 道具 → 動作，下面仍是軌道。

![時間軸流程](./src/assets/screen/7.png)

### 19. 進階預備

鎖定演員 → 分鏡靜圖 → 出片。連續鎖定，單格精修。

![進階預備](./src/assets/screen/6.png)

### 20. 設定 · 語言與通知

十種介面語言、淺色／深色／跟隨系統、完成後作業系統通知。

![設定語言與通知](./src/assets/screen/20.png)

### 21. 設定 · 供應商

預設本機 Grok 閘道，亦可選雲端或本機對話／圖像／影片。

![設定供應商](./src/assets/screen/21.png)
...
| **時間軸 Timeline** | 條件軌（鏡頭 + 參考）、snap／pack、生成工作區、即時編譯提示、綁定角色／場景／道具／**動作**、6s／10s 時長；**流程圖**顯示載入 → 提示 → 靜幀 → 影片 |
...
| **CLI `instant-drama`** | 本地 headless 或遠端 invoke；建置／開啟桌面 App；OpenClaw／Hermes agent（**190** 個 IPC channel） |
...
- 時間軸縮放、**Timeline snap**、snap grid、**Pack clips**、條件軌與生成工作區
- 流程顯示：**已編譯提示**、分階段流程圖（載入參考 → 提示 → 靜幀 → 影片）  
...
```bash
# Linux 範例
sudo dpkg -i release/instant-drama-magician_1.11.3_amd64.deb
# 或
...
instant-drama update install 1.11.3 --yes   # 釘選版本
```
...
instant-drama channels list --json          # 約 190 個 channel
```
...
| [docs/cli.md](./docs/cli.md) | [docs/cli-ZH.md](./docs/cli-ZH.md) | CLI（190 channels） |
