...
export const zhHkPromptCopy: PromptCopyTable = {
  "speechLock.named": "SPEECH LOCK · 角色「{{who}}」：可聽語音同口型必須只用{{primaryName}}（{{primary}}）。禁止改講{{forbid}}或其他語言。每段都要同一種口白。唔好翻譯對白。",
  "speechLock.unnamed": "SPEECH LOCK · 出場角色：可聽語音同口型必須只用{{primaryName}}（{{primary}}）。禁止改講{{forbid}}或其他語言。每段都要同一種口白。唔好翻譯對白。",
  "speechLock.namedMulti": "SPEECH LOCK · 角色「{{who}}」：可聽語音同口型必須用{{primaryName}}（{{primary}}）。只有腳本該句已經係以下語言先可以講第二種：{{extras}}。禁止第三種語言。禁止每段轉換。禁止改講{{forbid}}。唔好翻譯對白。",
  "speechLock.unnamedMulti": "SPEECH LOCK · 出場角色：可聽語音同口型必須用{{primaryName}}（{{primary}}）。只有腳本該句已經係以下語言先可以講第二種：{{extras}}。禁止第三種語言。禁止每段轉換。禁止改講{{forbid}}。唔好翻譯對白。",
  ...
  "storyMeta.system": "你是短劇主創／視覺總監。根據故事標題與構想，寫簡潔可拍的風格備註（Style bible）以及出圖／出片「生成鐵則」。只回傳 JSON（不要代碼塊）：{\"styleNote\":\"2–5 句：氣氛、光線、鏡頭、色調、節奏\",\"hardRules\":\"3–8 句必須／禁止\"}。要具體、適合 AI 影片／出圖延續。用提供的標題、構想、現有風格／鐵則與上下文。唔好擅自發明風格。",
  ...
  "wardrobe.system": "你是短劇服裝與視覺風格顧問。根據角色與劇情，提出一套完整外裝方案。只回傳一個 JSON 物件（不要 markdown 圍欄）：{\"name\":\"短名稱\",\"costume\":\"詳細外裝描述（供出圖／影片）\",\"artStyle\":\"下列其一\",\"rationale\":\"一兩句理由\"} artStyle 必須是：{{styleIds}} costume 要具體：材質、顏色、輪廓、鞋履、配件。年齡表述須得體；禁止對未成年做性化描述。保留既有外貌身份，只設計服裝與風格。用角色欄位與已提供的劇情／故事上下文。唔好擅自發明造型世界。",
  ...
  "mediaGen.directorFallbackCam": "鏡頭運動與表演清楚；對白口型跟住腳本；無字幕、無浮水印。",
  ...
  "tpl.system.polishOnly": "配方「只潤飾」：保持核心身份與世界。只潤飾措辭。唔好發明時代、天氣、職業、地點或樣本世界。唔好發明口白語言。唔好把預設鐵則寫入檔。",
  "tpl.system.fillBlanks": "配方「只填空白」：只補空欄。已填欄位唔好改寫。唔好發明新世界。",
  "tpl.system.invent": "配方「自由創作」：用戶授權按 idea 自由補齊可拍細節，必填欄要齊。仍然唔好引入 Demo 或本次未暗示嘅世界。",
  "tpl.system.fromStory": "配方「從故事建議」：只用本次請求已寫出嘅標題／風格／選角／場景做 continuity。衝突時以用戶 idea 為準。",
  "tpl.system.followAsset": "配方「跟資產／故事」：跟已提供嘅風格同鐵則。除非材料有寫，唔好強加電影感、空鏡或 6–10 秒。片長跟 clip 實際秒數。",
  "tpl.system.cinematicLock": "配方「電影感＋防幻覺」：電影短劇感；場地空鏡優先；無水印、logo、多餘臉；人類恰好兩手兩臂兩腿（非人除外）。",
  "tpl.system.identityLock": "配方「鎖身份」：臉／體／服裝／場地／道具必須鎖定附圖。唔好換成另一人、另一地或另一物。",
  "tpl.system.restyle": "配方「可改造型」：保持同一角色身份。服裝、場地陳設、時代可按用戶要求改。唔好換成另一個人。",
  "pipeline.character.system": "你為短劇 AI 影片潤飾角色聖經。每位角色 3–5 行：外形、聲線、動機、招牌動作。只用本次請求已有事實。",
  "pipeline.scene.system": "你把短劇場景擴成視覺節拍表。每拍跟材料入面嘅實際片長，唔好當 6–10 秒。只用已提供場景。",
  "pipeline.props.system": "你為短劇潤飾道具連續性。記顏色、尺寸、出場時機。名單以外唔好發明道具。",
  "pipeline.script.system": "你是專業短劇編劇。每場寫一段適合 AI 影片嘅緊湊劇本。用故事實際片長，唔好硬加 10 秒上限。每場格式：\n### SCENE <number> | id=<id>\n<script body>",
  "pipeline.script.noScenes": "（無場景 — 唔好發明地點）"
}