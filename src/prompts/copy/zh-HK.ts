...
export const zhHkPromptCopy: PromptCopyTable = {
  ...
  "storyMeta.system": "你是短劇主創／視覺總監。根據故事標題與構想，寫簡潔可拍的風格備註（Style bible）以及出圖／出片「生成鐵則」。只回傳 JSON（不要代碼塊）：{\"styleNote\":\"2–5 句：氣氛、光線、鏡頭、色調、節奏\",\"hardRules\":\"3–8 句必須／禁止\"}。要具體、適合 AI 影片／出圖延續。用提供的標題、構想、現有風格／鐵則與上下文；不足就自由補齊一套連貫風格與鐵則。",
  ...
  "wardrobe.system": "你是短劇服裝與視覺風格顧問。根據角色與劇情，提出一套完整外裝方案。只回傳一個 JSON 物件（不要 markdown 圍欄）：{\"name\":\"短名稱\",\"costume\":\"詳細外裝描述（供出圖／影片）\",\"artStyle\":\"下列其一\",\"rationale\":\"一兩句理由\"} artStyle 必須是：{{styleIds}} costume 要具體：材質、顏色、輪廓、鞋履、配件。年齡表述須得體；禁止對未成年做性化描述。保留既有外貌身份，只設計服裝與風格。用角色欄位與已提供的劇情／故事上下文；不足就自由補一套連貫造型。",
  ...
  "still.header": "SINGLE KEYFRAME STILL for short-drama video continuity（單格關鍵幀，不是多格板）。產出一張對應下方鏡頭的電影感主畫面。無文字、logo、浮水印、UI。清晰可製作。",
  ...
  "sceneIntro.camera": "運鏡：{{camera}}；連續輕微動態；空鏡為主——勿新增路人臉、logo、字幕。",
  ...
  "cover.posterLead": "PROFESSIONAL SHORT-DRAMA POSTER / KEY ART（16:9 電影感靜幀）。不是 UI 模型。無文字、logo、浮水印、標題字幕。",
