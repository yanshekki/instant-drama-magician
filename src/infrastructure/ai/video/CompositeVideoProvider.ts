...
export class CompositeVideoProvider implements VideoProvider {
  ...
  async generate(request: VideoGenRequest): Promise<VideoGenResult> {
    ...
    // auto: real HTTP only. Never write a purple ffmpeg placeholder — that
    // looks like a successful clip with the prompt burned as text.
    let probe
    try {
    ...
    if (!probe.available) {
      throw new AppError(
        'AI_UNAVAILABLE',
        'errors.videoUnavailable',
        probe.message || 'http-unavailable'
      )
    }
    this.lastProviderId = this.http.id
