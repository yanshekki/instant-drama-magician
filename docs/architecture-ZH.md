...
版本 **1.4.2**。Presentation → Application → Domain → Infrastructure，並以 **共用 handler runtime** 服務 Electron、Web 與 CLI。
...
Presentation（React 頁面／CLI／瀏覽器 UI）
  ...
  IPC  |  HTTP POST /api/invoke  |  instant-drama invoke
        │
        ▼
  registerAllHandlers + HandlerHost   ← 單一真相來源（約 158 channels）
        │
        ▼
...
Channel 目錄：`src/runtime/channelManifest.ts`（**158** 個唯一 id）。
