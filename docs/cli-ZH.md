...
**空間交換包（1.11.0）**加 **五條** channel（`spatial:compileBeat`、`spatial:importPackage`、`spatial:attachRef`、`spatial:blenderStatus`、`spatial:generateMesh`）——合計 **189**。白模靜圖鎖定分鏡靜圖的走位，再走現有圖生影片。可選貼圖平面 glTF 只是走位代理，不是成片。Blender 是命令列客戶端——見 [blender-ZH.md](./blender-ZH.md)。請用 `channels describe`，此處不羅列全部 channel。
