...
Version **1.5.0**. Presentation → Application → Domain → Infrastructure, with a **shared handler runtime** used by Electron, Web, and CLI.
...
Presentation (React pages / CLI / browser UI)
  ...
  IPC  |  HTTP POST /api/invoke  |  instant-drama invoke
        │
        ▼
  registerAllHandlers + HandlerHost   ← single source of truth (~167 channels)
        │
        ▼
...
Channel catalog: `src/runtime/channelManifest.ts` (**167** unique ids).
